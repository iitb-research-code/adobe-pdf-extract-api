...
def gen_tables(elements,page_dims):
  ...
  for pg,table,bound in table_names:
    ...
    for tr in trs:
      ...
      tds = [element['Path'] for element in elements if "TD" in element['Path'].split("/") and table in "/".join(element['Path'].split("/")) and tr in element['Path'].split("/") ]
...
for page in pages:
  ...
  for element in pages[page]:
    if element[0]=="Text":
      text = " ".join([str(i) for i in element[1]])
      hocr += f'      <span class="ocr_sent" title="bbox {text}">{element[2]}</span>\n'
    if element[0]=="Table":
      hocr += element[1]
    if element[0]=="Figure":
      img_path = element[2]
